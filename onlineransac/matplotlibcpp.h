...
class PythonEnvironment { // varevalo@uma.es
    public:
...
#       endif
    ...
    private:
        ...
        {
            char* python_home = std::getenv("PYTHON_HOME"); // First try
            if (!python_home) {

                python_home = std::getenv("PYTHONPATH"); // Another try
                if (!python_home) {
                    throw invalid_argument("PYTHON_HOME environment variable not set");
                }
            }
...
#if PY_MAJOR_VERSION >= 3
        wchar_t progname[] = L"plotting";
#else
        char progname[] = "plotting";
#endif

        wchar_t const *dummy_args[] = {L"Python", NULL};  // const is needed because literals must not be modified

// varevalo@uma.es
#if PY_VERSION_HEX < 0x030B0000 // old version
        ...
        // Setup parameters
    #if PY_MAJOR_VERSION >= 3
        PySys_SetArgv(1, const_cast<wchar_t **>(dummy_args));
    #else
        PySys_SetArgv(1, const_cast<char **>(dummy_args));
    #endif
        ...
        // Setup parameters
        PyConfig_SetArgv(&config, 1, const_cast<wchar_t **>(dummy_args));
