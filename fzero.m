...
allOptionsDefault = nargin < 3;
if allOptionsDefault
    % Most common case - no options passed in.
    % Flag to indicate minimal options validation
    options = [];
    if nargin == 1
...
if ~allOptionsDefault
    % Options passed in - run validation
...
if funValCheck
    ...
    FunFcn = @(x, varargin) matlab.internal.optimfun.utils.checkfun(x, FunFcn, "fzero", varargin{:});
end
...
elseif isscalar(x) % Starting guess scalar input
    if trace > 2 
        disp(' ')
...
%--------------------------------------------------------------------------
function localFirstFcnEvalError(FunFcn,FunFcnIn,ME)
